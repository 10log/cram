...
var Tt = {
	...
	exit() {
		...
		let e = this._renderer, { targetUuid: n, savedCameraPosition: r, savedCameraQuaternion: i, savedControlsTarget: a, initialContainerSave: c } = this.state;
		this.detachMouseListeners(), this.detachPropertyListeners(), e.overlays.global.removeCell("fpv-mode"), this.removeOrbitButton();
		let l = o.getState().containers[n];
		if (l) {
			let e = l.save(), r = c;
			mt({
				...
				objectId: n,
				recallFunction: (i) => {
					let a = o.getState().containers[n];
					a && (i === "UNDO" ? a.restore(r) : a.restore(e), o.getState().set((e) => {
						e.version++;
					}), t("RENDER"));
				}
			});
	...
	handlePropertyChange = (n) => {
		if (!this.state || n.uuid !== this.state.targetUuid || !e.ROTATION_PROPS.has(n.property)) return;
		let r = o.getState().containers[this.state.targetUuid];
		if (!r) return;
		let i = new u.Vector3(0, 0, 1).applyEuler(r.rotation);
		this.extractYawPitch(i), this._renderer.camera.quaternion.copy(this.buildQuaternion()), this.syncOrientationControl(), t("RENDER");
	};
	attachPropertyListeners() {
		this.propertyUnsubs.push(r("SOURCE_SET_PROPERTY", this.handlePropertyChange), r("RECEIVER_SET_PROPERTY", this.handlePropertyChange));
	}
	...
	handleMouseMove(e) {
		...
		let r = e.clientX - this.lastMouseX, i = e.clientY - this.lastMouseY;
		this.lastMouseX = e.clientX, this.lastMouseY = e.clientY, this.yaw += -r * Pt, this.pitch = Math.max(-Math.PI / 2 + .001, Math.min(Math.PI / 2 - .001, this.pitch + -i * Pt)), this._renderer.camera.quaternion.copy(this.buildQuaternion()), this.syncCameraToContainer(n), this.syncOrientationControl();
	...
	showOrbitButton() {
		...
		n.textContent = "Back to Orbit", Object.assign(n.style, {
			position: "absolute",
			bottom: "12px",
			left: "50%",
			...
			n.style.background = "rgba(255,255,255,0.25)";
		}), n.addEventListener("mouseleave", () => {
			n.style.background = "rgba(255,255,255,0.12)";
		}), n.addEventListener("click", () => {
			t("EXIT_FIRST_PERSON");
		}), e.appendChild(n), this.orbitButton = n;
	}
	removeOrbitButton() {
...
var Yt = {
	...
	init(e) {
		...
		let n = c.getState().theme.renderer.background;
		this.scene = new u.Scene(), this.scene.background = new u.Color(n), this.scene.add(this.env, this.workspace, this.interactables, this.fdtdItems, this.sketches, this.markup), this.renderer = new u.WebGLRenderer({
			canvas: this.elt,
		...
		this.renderer.setPixelRatio(r), this._fov = 45;
		let a = this.aspect, o = [
			0,
			0,
			1
		];
		this._camera = new u.PerspectiveCamera(this._fov, a, .01, 500), this._camera.layers.enableAll(), this._camera.position.set(1, 1, 1), this._camera.up.set(o[0], o[1], o[2]), this.mouseConfigSet = {
			Default: {
				LEFT: u.MOUSE.ROTATE,
		...
		}), i.addMessageHandler("TOGGLE_RENDERER_STATS_VISIBLE", () => {
			this.stats.hidden ? this.stats.unhide() : this.stats.hide();
		}), i.addMessageHandler("GET_RENDERER_STATS_VISIBLE", () => !this.stats.hidden), this.addCleanupListener(window, "mouseup", (e) => {
		...
		this.orientationControl = new Mt("#orientation-overlay", {
			...
			e.target.match(/top|bottom|right|left|front|back/gi) && i.postMessage("LOOK_ALONG_AXIS", e.target);
	...
	render() {
		...
		e && (this.composer.render(), this.updateCursorSize(), this.updateStats(), this.orientationControl.shouldRender = !0, i.postMessage("RENDERER_UPDATED"), t("RENDERER_UPDATED", void 0), this.needsToRender = !1), this.orientationControl.shouldRender && this.orientationControl.render(), e || this.orientationControl.shouldRender ? (this.isIdle = !1, requestAnimationFrame(this.render)) : (this.isIdle = !0, setTimeout(() => requestAnimationFrame(this.render), 50));
	}
	...
	get mode() {
		return i.postMessage("GET_EDITOR_MODE")[0];
	}
	cleanupFunctions = [];
...
a("RENDER", () => {
	$.requestRender();
}), a("RENDERER_SHOULD_ANIMATE", (e) => {
	$.shouldAnimate = e, e && $.requestRender();
}), a("PHASE_OUT", () => {
	$.firstPersonControls?.active ? t("EXIT_FIRST_PERSON") : $.isPerformingOperation ? t("STOP_OPERATIONS") : t("DESELECT_ALL_OBJECTS"), $.requestRender();
}), a("STOP_OPERATIONS", () => {
...
}), a("TOGGLE_CAMERA_ORTHO", () => {
	$.isOrtho = !$.isOrtho;
}), a("FOCUS_ON_SELECTED_OBJECTS", () => {
	let e = [...o.getState().selectedObjects];
	if (e && e.length > 0) {
