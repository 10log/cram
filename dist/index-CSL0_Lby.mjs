import { S as V } from "./solver-CoMY3Alv.mjs";
import { v as I, e as S, R as M, q as k, r as v, p as U, k as f, s as A, t as T, w as q, L as b, x as p, y as F, z as H, F as z, M as G, A as w, V as R, B as j, I as W, C as N, o as m, D as Y, E as B, G as $, f as y } from "./index-gsnxGZDe.mjs";
import { T as _ } from "./THREE.MeshLine-DkUU-3bc.mjs";
function K() {
  let o = [];
  const e = new _.MeshLine();
  e.setPoints(o);
  const t = new _.MeshLineMaterial({
    lineWidth: 0.1,
    color: 16711680,
    sizeAttenuation: 1
  });
...
class Q extends V {
  ...
  updateImageSourceCalculation() {
    ...
    let e = {
      baseSource: f.getState().containers[this.sourceIDs[0]],
      position: f.getState().containers[this.sourceIDs[0]].position.clone(),
      room: this.room,
      reflector: null,
      parent: null,
      order: 0
    ...
    let s, i = [];
    if (r !== null) {
      s = r.constructPathsForAllDescendents(f.getState().containers[this.receiverIDs[0]]), this.allRayPaths = s;
  ...
  async calculateImpulseResponse() {
    ...
    if (i == null || i.sort((a, n) => a.arrivalTime(343) > n.arrivalTime(343) ? 1 : -1), console.log(i), i != null) {
      ...
      for (let h = 0; h < i.length; h++) {
        let x = i[h].arrivalTime(343), O = i[h].arrivalPressure(s, this.frequencies);
        Math.random() > 0.5 && (O = O.map((P) => -P));
        let C = Math.floor(x * r);
  ...
  async downloadImpulseResponse(e, t = 44100) {
    ...
    const r = F([H(this.impulseResponse.getChannelData(0))], { sampleRate: t, bitDepth: 32 }), s = e.endsWith(".wav") ? "" : ".wav";
    z.saveAs(r, e + s);
  }
  // getters and setters
  get sources() {
  ...
  get receivers() {
    return this.receiverIDs.length > 0 && Object.keys(f.getState().containers).length > 0 ? this.receiverIDs.map((e) => f.getState().containers[e]) : [];
  }
  get room() {
...
m("IMAGESOURCE_SET_PROPERTY", Y);
m("REMOVE_IMAGESOURCE", B);
m("ADD_IMAGESOURCE", $(Q));
...
m("IMAGESOURCE_DOWNLOAD_IR", (o) => {
  var l, a;
  const e = y.getState().solvers[o], t = f.getState().containers, r = e.sourceIDs.length > 0 && ((l = t[e.sourceIDs[0]]) == null ? void 0 : l.name) || "source", s = e.receiverIDs.length > 0 && ((a = t[e.receiverIDs[0]]) == null ? void 0 : a.name) || "receiver", i = `ir-imagesource-${r}-${s}`.replace(/[^a-zA-Z0-9-_]/g, "_");
...
//# sourceMappingURL=index-CSL0_Lby.mjs.map
