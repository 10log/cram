...
var r = (g, o, e) => N(g, typeof o != "symbol" ? o + "" : o, e);
import { M as X, k as j, r as y, N as Y, O as z, o as G, Q as O, U as M } from "./index-BovKYX7u.mjs";
...
import { S as ee } from "./solver-cvNYXUbR.mjs";
class te {
  /**
  ...
  constructor(o, e, t) {
    ...
    this.setDataType = function(a) {
      return i = a, this;
    }, this.addVariable = function(a, n, h) {
      const m = this.createShaderMaterial(n), d = {
        name: a,
    ...
    }, this.setVariableDependencies = function(a, n) {
      a.dependencies = n;
    }, this.init = function() {
      if (t.capabilities.maxVertexTextures === 0)
        return "No support for vertex shader textures.";
      for (let a = 0; a < this.variables.length; a++) {
        const n = this.variables[a];
        n.renderTargets[0] = this.createRenderTarget(o, e, n.wrapS, n.wrapT, n.minFilter, n.magFilter), n.renderTargets[1] = this.createRenderTarget(o, e, n.wrapS, n.wrapT, n.minFilter, n.magFilter), this.renderTexture(n.initialValueTexture, n.renderTargets[0]), this.renderTexture(n.initialValueTexture, n.renderTargets[1]);
        const h = n.material, m = h.uniforms;
        if (n.dependencies !== null)
          for (let d = 0; d < n.dependencies.length; d++) {
            const x = n.dependencies[d];
            if (x.name !== n.name) {
              let T = !1;
              ...
              if (!T)
                return "Variable dependency not found. Variable=" + n.name + ", dependency=" + x.name;
            }
...
` + h.fragmentShader;
      ...
      const a = this.currentTextureIndex, n = this.currentTextureIndex === 0 ? 1 : 0;
      for (let h = 0, m = this.variables.length; h < m; h++) {
        ...
        this.doRenderTarget(d.material, d.renderTargets[n]);
      }
    ...
    this.createShaderMaterial = u, this.createRenderTarget = function(a, n, h, m, d, x) {
      return a = a || o, n = n || e, h = h || D, m = m || D, d = d || w, x = x || w, new k(a, n, {
        wrapS: h,
        wrapT: m,
        minFilter: d,
        magFilter: x,
      ...
      const a = new Float32Array(o * e * 4), n = new q(a, o, e, R, P);
      return n.needsUpdate = !0, n;
    }, this.renderTexture = function(a, n) {
      s.passThruTexture.value = a, this.doRenderTarget(l, n), s.passThruTexture.value = null;
    }, this.doRenderTarget = function(a, n) {
      const h = t.getRenderTarget(), m = t.xr.enabled, d = t.shadowMap.autoUpdate;
      t.xr.enabled = !1, t.shadowMap.autoUpdate = !1, c.material = a, t.setRenderTarget(n), c.render(t), c.material = l, t.xr.enabled = m, t.shadowMap.autoUpdate = d, t.setRenderTarget(h);
    };
    function p() {
      return `void main()	{
...
}`, se = `uniform sampler2D clearTexture;
...
`, re = `uniform sampler2D heightmap;
uniform float inv_cell_size;
uniform float cell_size;
...
function I(g, o, e, t) {
  const i = [];
  let s, l, c, f, u, p, v, a, n, h, m;
  if (c = e - g, f = t - o, u = Math.abs(c), p = Math.abs(f), v = 2 * p - u, a = 2 * u - p, p <= u)
    for (c >= 0 ? (s = g, l = o, n = e) : (s = e, l = t, n = g), i.push([s, l]), m = 0; s < n; m++)
      s = s + 1, v < 0 ? v = v + 2 * p : (c < 0 && f < 0 || c > 0 && f > 0 ? l = l + 1 : l = l - 1, v = v + 2 * (p - u)), i.push([s, l]);
  else
    for (f >= 0 ? (s = g, l = o, h = t) : (s = e, l = t, h = o), i.push([s, l]), m = 0; l < h; m++)
      l = l + 1, a <= 0 ? a = a + 2 * u : (c < 0 && f < 0 || c > 0 && f > 0 ? s = s + 1 : s = s - 1, a = a + 2 * (u - p)), i.push([s, l]);
  return i;
}
class U {
  constructor(o) {
    ...
    r(this, "y2");
    r(this, "cells");
    r(this, "previousCells");
    r(this, "shouldClearPreviousCells");
...
class de extends ee {
  constructor(e) {
    ...
    r(this, "mesh");
    r(this, "editMesh");
    r(this, "heightmapVariable");
    r(this, "sourcemapVariable");
    r(this, "sourcemap");
    r(this, "readLevelShader");
    r(this, "readLevelImage");
    r(this, "readLevelRenderTarget");
    r(this, "sources");
    r(this, "sourceKeys");
    r(this, "receivers");
    r(this, "receiverKeys");
  ...
  init() {
    ...
    ]), s = C.waterVert, l = C.waterFrag, c = F, f = new A({
      uniforms: i,
      vertexShader: s,
      fragmentShader: l,
      side: c,
  ...
  addWall(e) {
    const t = b(Math.floor((e.x1 - this.offsetX) / this.cellSize), 0, this.nx - 1), i = b(Math.floor((e.y1 - this.offsetY) / this.cellSize), 0, this.nx - 1), s = b(Math.floor((e.x2 - this.offsetX) / this.cellSize), 0, this.nx - 1), l = b(Math.floor((e.y2 - this.offsetY) / this.cellSize), 0, this.nx - 1);
    this.walls.push(new U({ x1: t, y1: i, x2: s, y2: l })), this.updateWalls();
  }
  addWallsFromSurfaceEdges(e) {
    const t = e.edges.geometry.getAttribute("position");
    for (let i = 0; i < t.count; i += 2) {
      let s = b(Math.floor((t.getX(i) - this.offsetX) / this.cellSize), 0, this.nx - 1), l = b(Math.floor((t.getY(i) - this.offsetY) / this.cellSize), 0, this.ny - 1), c = b(Math.floor((t.getX(i + 1) - this.offsetX) / this.cellSize), 0, this.nx - 1), f = b(Math.floor((t.getY(i + 1) - this.offsetY) / this.cellSize), 0, this.ny - 1);
      this.walls.push(new U({ x1: s, y1: l, x2: c, y2: f }));
    }
    this.updateWalls();
  }
  fillSourceTexture() {
      ...
      for (let t = 0; t < this.walls.length; t++) {
        if (this.walls[t].shouldClearPreviousCells) {
          for (let i = 0; i < this.walls[t].previousCells.length; i++) {
            const s = 4 * (this.walls[t].previousCells[i][1] * this.nx + this.walls[t].previousCells[i][0]);
            e[s + 2] = 1;
          }
        ...
        if (this.walls[t].enabled)
          for (let i = 0; i < this.walls[t].cells.length; i++) {
            const s = 4 * (this.walls[t].cells[i][1] * this.nx + this.walls[t].cells[i][0]);
            e[s + 2] = 0;
          }
        else
          for (let i = 0; i < this.walls[t].cells.length; i++) {
  ...
  updateSourceTexture() {
    ...
    if (e) {
      for (let t = 0; t < this.sourceKeys.length; t++) {
        const i = Math.round((this.sources[this.sourceKeys[t]].x - this.offsetX) / this.cellSize), l = 4 * (Math.round((this.sources[this.sourceKeys[t]].y - this.offsetY) / this.cellSize) * this.nx + i);
  ...
  fillTexture(e) {
    ...
    if (!t) return;
    let i = 0;
    for (let s = 0; s < this.ny; s++)
      for (let l = 0; l < this.nx; l++)
        t[i + 0] = M(0, -2, 2, 0, 255), t[i + 1] = 0, t[i + 2] = 1, t[i + 3] = 1, i += 4;
  }
  readReceiverLevels() {
    ...
    for (let t = 0; t < this.receiverKeys.length; t++) {
      ...
      if (this.receivers[i]) {
        const s = (this.receivers[i].position.x - this.offsetX) / this.width, l = (this.receivers[i].position.y - this.offsetY) / this.height;
        this.readLevelShader.uniforms.point1.value.set(s, l), this.gpuCompute.doRenderTarget(this.readLevelShader, this.readLevelRenderTarget), y.renderer.readRenderTargetPixels(
        ...
        this.receivers[i].fdtdSamples.push((f - 127.5) / 127.5);
      }
    }
  }
  clear() {
...
//# sourceMappingURL=index-Cn8BarjH.mjs.map
