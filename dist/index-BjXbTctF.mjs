import { O as k, k as B, r as T, Q as j, U as V, o as N, V as X, W as z } from "./index-CgcZvXNw.mjs";
import { NearestFilter as y, ShaderMaterial as U, WebGLRenderTarget as A, FloatType as L, RGBAFormat as F, DataTexture as Y, ClampToEdgeWrapping as D, PlaneGeometry as W, MeshBasicMaterial as G, MeshLambertMaterial as O, DoubleSide as M, Mesh as P, UniformsUtils as q, UniformsLib as p, Color as K, Vector2 as E, UnsignedByteType as Q } from "three";
import { S as J } from "./solver-BGevQtbj.mjs";
class Z {
  /**
...
function H(v, e, t, i) {
  const a = [];
  let r, l, u, h, f, m, x, s, n, o, d;
  if (u = t - v, h = i - e, f = Math.abs(u), m = Math.abs(h), x = 2 * m - f, s = 2 * f - m, m <= f)
    for (u >= 0 ? (r = v, l = e, n = t) : (r = t, l = i, n = v), a.push([r, l]), d = 0; r < n; d++)
      r = r + 1, x < 0 ? x = x + 2 * m : (u < 0 && h < 0 || u > 0 && h > 0 ? l = l + 1 : l = l - 1, x = x + 2 * (m - f)), a.push([r, l]);
  else
    for (h >= 0 ? (r = v, l = e, o = i) : (r = t, l = i, o = e), a.push([r, l]), d = 0; l < o; d++)
      l = l + 1, s <= 0 ? s = s + 2 * f : (u < 0 && h < 0 || u > 0 && h > 0 ? r = r + 1 : r = r - 1, s = s + 2 * (f - m)), a.push([r, l]);
  return a;
}
class I {
  constructor(e) {
...
class oe extends J {
  constructor(e) {
    ...
    const t = [...B.getState().selectedObjects.values()].filter((h) => h.kind === "surface");
    let i = null;
    if (e = e || {}, t.length > 0) {
    ...
    this.editMesh = new P(l, u[0]), this.editMesh.name = "fdtd-2d-edit-mesh", this.editMesh.visible = !1, T.fdtdItems.add(this.editMesh), this.fillTexture = this.fillTexture.bind(this), this.init = this.init.bind(this), this.render = this.render.bind(this), this.updateWalls = this.updateWalls.bind(this), this.updateSourceTexture = this.updateSourceTexture.bind(this), this.addWallsFromSurfaceEdges = this.addWallsFromSurfaceEdges.bind(this), this.setWireframeVisible = this.setWireframeVisible.bind(this), this.getWireframeVisible = this.getWireframeVisible.bind(this), this.toggleWall = this.toggleWall.bind(this), this.clear = this.clear.bind(this), this.init(), this.onModeChange(j("GET_EDITOR_MODE")[0]), i && this.addWallsFromSurfaceEdges(i);
  }
  onModeChange(e) {
    switch (e) {
      case V.OBJECT:
  ...
  init() {
    ...
    ]), a = S.waterVert, r = S.waterFrag, l = M, u = new U({
      uniforms: i,
      vertexShader: a,
      fragmentShader: r,
      side: l,
    ...
    u.lights = !0, this.uniforms = u.uniforms, this.mesh = new P(e, u), this.mesh.matrixAutoUpdate = !1, this.mesh.updateMatrix(), this.mesh.material.wireframe = !1, this.mesh.matrixAutoUpdate = !0, this.mesh.scale.setZ(0.01), T.fdtdItems.add(this.mesh), this.gpuCompute = new Z(this.nx, this.ny, T.renderer);
  ...
  dispose() {
    ...
    for (let e = 0; e < this.messageHandlers.length; e++)
      X(this.messageHandlers[e][0], this.messageHandlers[e][1]);
  ...
  addWall(e) {
    const t = _(Math.floor((e.x1 - this.offsetX) / this.cellSize), 0, this.nx - 1), i = _(Math.floor((e.y1 - this.offsetY) / this.cellSize), 0, this.nx - 1), a = _(Math.floor((e.x2 - this.offsetX) / this.cellSize), 0, this.nx - 1), r = _(Math.floor((e.y2 - this.offsetY) / this.cellSize), 0, this.nx - 1);
...
//# sourceMappingURL=index-BjXbTctF.mjs.map
