import { Z as k, _ as y, $ as U, a0 as A, a1 as L, a2 as F, a3 as B, a4 as D, k as j, a5 as P, a6 as N, a7 as X, a8 as V, M as W, r as T, a9 as Y, aa as z, ab as G, ac as p, ad as K, ae as E, af as O, o as q, ag as Z, ah as M } from "./index-gsnxGZDe.mjs";
import { S as J } from "./solver-CoMY3Alv.mjs";
class Q {
  /**
        ...
        if (n.dependencies !== null)
          for (let c = 0; c < n.dependencies.length; c++) {
            ...
            if (g.name !== n.name) {
              ...
              if (!b)
                return "Variable dependency not found. Variable=" + n.name + ", dependency=" + g.name;
            }
...
` + o.fragmentShader;
      ...
      for (let o = 0, d = this.variables.length; o < d; o++) {
        ...
        if (c.dependencies !== null) {
          ...
          for (let b = 0, w = c.dependencies.length; b < w; b++) {
            ...
            g[R.name].value = R.renderTargets[s].texture;
          }
        }
    ...
    this.createShaderMaterial = f, this.createRenderTarget = function(s, n, o, d, c, g) {
      return s = s || e, n = n || t, o = o || D, d = d || D, c = c || y, g = g || y, new A(s, n, {
        wrapS: o,
        wrapT: d,
        minFilter: c,
        magFilter: g,
      ...
      const s = new Float32Array(e * t * 4), n = new B(s, e, t, F, L);
      return n.needsUpdate = !0, n;
    }, this.renderTexture = function(s, n) {
      a.passThruTexture.value = s, this.doRenderTarget(l, n), a.passThruTexture.value = null;
...
function H(v, e, t, i) {
  const r = [];
  let a, l, u, h, f, m, _, s, n, o, d;
  if (u = t - v, h = i - e, f = Math.abs(u), m = Math.abs(h), _ = 2 * m - f, s = 2 * f - m, m <= f)
    for (u >= 0 ? (a = v, l = e, n = t) : (a = t, l = i, n = v), r.push([a, l]), d = 0; a < n; d++)
      a = a + 1, _ < 0 ? _ = _ + 2 * m : (u < 0 && h < 0 || u > 0 && h > 0 ? l = l + 1 : l = l - 1, _ = _ + 2 * (m - f)), r.push([a, l]);
  else
    for (h >= 0 ? (a = v, l = e, o = i) : (a = t, l = i, o = e), r.push([a, l]), d = 0; l < o; d++)
      l = l + 1, s <= 0 ? s = s + 2 * f : (u < 0 && h < 0 || u > 0 && h > 0 ? a = a + 1 : a = a - 1, s = s + 2 * (f - m)), r.push([a, l]);
  return r;
}
class I {
  constructor(e) {
...
class le extends J {
  constructor(e) {
    ...
    const t = [...j.getState().selectedObjects.values()].filter((h) => h.kind === "surface");
    let i = null;
    if (e = e || {}, t.length > 0) {
  ...
  init() {
    ...
    ]), r = S.waterVert, a = S.waterFrag, l = V, u = new U({
      uniforms: i,
      vertexShader: r,
      fragmentShader: a,
      side: l,
    ...
    u.lights = !0, this.uniforms = u.uniforms, this.mesh = new W(e, u), this.mesh.matrixAutoUpdate = !1, this.mesh.updateMatrix(), this.mesh.material.wireframe = !1, this.mesh.matrixAutoUpdate = !0, this.mesh.scale.setZ(0.01), T.fdtdItems.add(this.mesh), this.gpuCompute = new Q(this.nx, this.ny, T.renderer);
    ...
    f !== null && console.error(f), this.clearShader = this.gpuCompute.createShaderMaterial(S.clearFrag, { clearTexture: { value: null } }), this.readLevelShader = this.gpuCompute.createShaderMaterial(S.readLevelFrag, {
      ...
      stencilBuffer: !1,
      depthBuffer: !1
    }), this.eventListeners.push(q("RENDERER_UPDATED", () => {
  ...
  dispose() {
    ...
    for (let e = 0; e < this.messageHandlers.length; e++)
      Z(this.messageHandlers[e][0], this.messageHandlers[e][1]);
  ...
  addWall(e) {
    const t = x(Math.floor((e.x1 - this.offsetX) / this.cellSize), 0, this.nx - 1), i = x(Math.floor((e.y1 - this.offsetY) / this.cellSize), 0, this.nx - 1), r = x(Math.floor((e.x2 - this.offsetX) / this.cellSize), 0, this.nx - 1), a = x(Math.floor((e.y2 - this.offsetY) / this.cellSize), 0, this.nx - 1);
    this.walls.push(new I({ x1: t, y1: i, x2: r, y2: a })), this.updateWalls();
  }
  addWallsFromSurfaceEdges(e) {
    const t = e.edges.geometry.getAttribute("position");
    for (let i = 0; i < t.count; i += 2) {
      let r = x(Math.floor((t.getX(i) - this.offsetX) / this.cellSize), 0, this.nx - 1), a = x(Math.floor((t.getY(i) - this.offsetY) / this.cellSize), 0, this.ny - 1), l = x(Math.floor((t.getX(i + 1) - this.offsetX) / this.cellSize), 0, this.nx - 1), u = x(Math.floor((t.getY(i + 1) - this.offsetY) / this.cellSize), 0, this.ny - 1);
      this.walls.push(new I({ x1: r, y1: a, x2: l, y2: u }));
    }
    this.updateWalls();
  }
  fillSourceTexture() {
      ...
      for (let t = 0; t < this.walls.length; t++) {
        if (this.walls[t].shouldClearPreviousCells) {
          for (let i = 0; i < this.walls[t].previousCells.length; i++) {
            const r = 4 * (this.walls[t].previousCells[i][1] * this.nx + this.walls[t].previousCells[i][0]);
            e[r + 2] = 1;
          }
        ...
        if (this.walls[t].enabled)
          for (let i = 0; i < this.walls[t].cells.length; i++) {
            const r = 4 * (this.walls[t].cells[i][1] * this.nx + this.walls[t].cells[i][0]);
            e[r + 2] = 0;
          }
        else
          for (let i = 0; i < this.walls[t].cells.length; i++) {
  ...
  updateSourceTexture() {
    ...
    if (e) {
      for (let t = 0; t < this.sourceKeys.length; t++) {
        const i = Math.round((this.sources[this.sourceKeys[t]].x - this.offsetX) / this.cellSize), a = 4 * (Math.round((this.sources[this.sourceKeys[t]].y - this.offsetY) / this.cellSize) * this.nx + i);
        ...
        if (e[a + 0] = M(l, -2, 2, 0, 255), e[a + 1] = M(u, -2, 2, 0, 255), e[a + 3] = 0, this.sources[this.sourceKeys[t]].shouldClearPreviousPosition) {
  ...
  readReceiverLevels() {
    ...
    for (let t = 0; t < this.receiverKeys.length; t++) {
      ...
      if (this.receivers[i]) {
        const r = (this.receivers[i].position.x - this.offsetX) / this.width, a = (this.receivers[i].position.y - this.offsetY) / this.height;
        this.readLevelShader.uniforms.point1.value.set(r, a), this.gpuCompute.doRenderTarget(this.readLevelShader, this.readLevelRenderTarget), T.renderer.readRenderTargetPixels(
...
//# sourceMappingURL=index-4TxntXMQ.mjs.map
