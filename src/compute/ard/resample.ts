 ...
 */
export function kaiserWindow(t: number, beta: number): number {
  if (t <= -1 || t >= 1) return 0;
...
export const DEFAULT_ZERO_CROSSINGS = 16;
export const DEFAULT_KAISER_BETA = 8;
...
export function resample(
  ...
  const { zeroCrossings = DEFAULT_ZERO_CROSSINGS, beta = DEFAULT_KAISER_BETA } = options;

  if (!(fromRate > 0) || !(toRate > 0)) {
    throw new Error(`Sample rates must be positive, got ${fromRate} and ${toRate}`);
  }
  if (!Number.isInteger(zeroCrossings) || zeroCrossings < 1) {
    throw new Error(`zeroCrossings must be a positive integer, got ${zeroCrossings}`);
  }
  ...
  // Cutoff relative to the input Nyquist. Upsampling keeps the whole input
  // band; downsampling has to cut at the *output* Nyquist or the discarded
  // band folds back as aliasing.
  const cutoff = Math.min(1, ratio);
  // Half-width in input samples. Dividing by the cutoff keeps the same number
  // of zero crossings when the kernel is stretched for decimation.
  const halfWidth = zeroCrossings / cutoff;

  const out = new Float32Array(outLength);
  const inLast = input.length - 1;

  for (let m = 0; m < outLength; m++) {
    // Position in input samples of output sample m.
    const centre = m / ratio;
    const lo = Math.max(0, Math.ceil(centre - halfWidth));
    const hi = Math.min(inLast, Math.floor(centre + halfWidth));

    let sum = 0;
    for (let n = lo; n <= hi; n++) {
      const u = centre - n;
      sum += input[n] * cutoff * sinc(cutoff * u) * kaiserWindow(u / halfWidth, beta);
    }
    out[m] = sum;
  }
